...
# RUTA AL EXCEL
# ─────────────────────────────────────────────────────────────
CARPETA_TIENDA = r'C:\Users\PC\Desktop\TIENDA de INVENTARIO_NEVERAS'
EXCEL_PATH = os.path.join(CARPETA_TIENDA, 'INVENTARIO_NEVERAS.xlsx')
if not os.path.exists(EXCEL_PATH):
    # En Vercel no se incluyen datos operativos locales. La plantilla mantiene
    # la estructura hasta que se restaure un respaldo en Google Sheets.
    EXCEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'plantilla_neveras.xlsx')


if not os.path.exists(EXCEL_PATH):
    BASE_DIR   = os.path.dirname(os.path.abspath(__file__))
    EXCEL_PATH = os.path.join(BASE_DIR, 'INVENTARIO_NEVERAS.xlsx')
...
def _precio_de_inventario(producto):
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row[0] and str(row[0]) == producto:
...
def leer_ventas(prods_base=None):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        if not row[1]:
            continue
        num      = row[0]
        cantidad = int(row[5] or 0)
        producto = str(row[4] or '')
        precio   = float(row[6] or 0)
        total    = float(row[7] or 0)
        ganancia = float(row[11] or 0)
        ...
        ventas.append({
            ...
            'fecha':    str(row[1])[:10],
            'hora':     str(row[2] or ''),
            'cliente':  str(row[3] or '').upper(),
            'producto': producto,
            'cantidad': cantidad,
            'precio':   precio,
            'total':    total,
            'metodo':   str(row[8] or ''),
            'pago':     'SI' if str(row[9] or '').upper() == 'SI' else 'NO',
...
def leer_inventario_base():
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        prods.append({
            ...
            'costo':    float(row[1] or 0),
            'precio':   float(row[2] or 0),
            'stockIni': float(row[4] or 0),
            'stockMin': float(row[5] or 0),
        })
    wb.close()
    return prods
...
def _static(nombre):
    ruta = os.path.join(CARPETA_TIENDA, nombre)
...
if __name__ == '__main__':
    sep = '=' * 60
    print(sep)
    print('  🧊  Dashboard Neveras — Servidor')
    print(sep)
    if os.path.exists(EXCEL_PATH):
        print(f'  ✅ Excel encontrado:')
        print(f'     {EXCEL_PATH}')
    else:
        print(f'  ❌ Excel NO encontrado en:')
        print(f'     {EXCEL_PATH}')
    print()
    print('  🌐  http://localhost:5000')
    print('  🔄  Recarga la página para leer cambios del Excel')
    print('  ⚠️   Cierra el Excel antes de registrar ventas')
