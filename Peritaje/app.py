...
EXCEL_FILE = os.path.join(os.path.dirname(__file__), 'data', 'peritajes.xlsx')
...
def init_excel():
    if not sheets_enabled() and not os.path.exists(EXCEL_FILE):
        wb = Workbook()
        ws = wb.active
        ws.title = "Peritajes"
        headers = [
            "N° Peritaje", "Fecha", "N° OT", "Cliente", "Placa",
...
def get_next_number():
    ...
    return ws.max_row  # header is row 1, so max_row = count of data + 1
...
if __name__ == '__main__':
    init_excel()
    print("\n" + "="*50)
    print("  VENE AUTOS - Sistema de Peritaje")
